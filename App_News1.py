...
def fetch_news(query):
    ...
    for name, cfg in api_mapping.items():
        try:
            ...
            if name == "NewsAPI":
                raw_articles = data.get("articles", [])[:2]
            elif name == "CurrentsAPI":
                raw_articles = data.get("news", [])[:2]
            elif name == "GNews":
                raw_articles = data.get("articles", [])[:2]
            elif name == "Newsdata":
                raw_articles = data.get("results", []) [:2]
            elif name == "Marketaux":
                raw_articles = data.get("data", [])[:2] if isinstance(data, dict) else []
            elif name == "NYTimes":
...
if "sentiment_chart_shown" not in st.session_state:
    with st.spinner("Loading sentiment insights..."):
