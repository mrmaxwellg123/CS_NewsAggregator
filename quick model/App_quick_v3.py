...
if query_terms:
    ...
    if all_articles:
        for idx, article in enumerate(all_articles[:10]):
            with st.container():
                ...
                if st.button(f"🔗 {article['title']}", key=f"article_{idx}"):
                    main_category = categorize_article(article['title']) or "Uncategorized"
                    st.session_state.category_clicks[main_category] += 1
